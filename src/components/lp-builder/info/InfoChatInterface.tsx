...
type InfoChatInterfaceProps = {
  lpId: string;
};

export default function InfoChatInterface({ lpId }: InfoChatInterfaceProps) {
  const router = useRouter();
  const { toast } = useToast();
  const { state, setChatMessages, completePhase } = useLPBuilder();
  const [messages, setMessages] = useState<Message[]>(state.chatMessages.length ? state.chatMessages : [
    {
      id: 1,
      role: 'system',
