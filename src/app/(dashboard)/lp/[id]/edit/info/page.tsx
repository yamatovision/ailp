...
import InfoChatInterface from '@/components/lp-builder/info/InfoChatInterface';
import { LPBuilderProvider } from '@/components/lp-builder/LPBuilderContext';

export default function LPInfoPage({ params }: { params: { id: string } }) {
  ...
  return (
    <LPBuilderProvider initialLpId={params.id} initialTitle={title}>
      <LPBuilderLayout currentPhase="info" lpId={params.id} title={title}>
        <InfoChatInterface lpId={params.id} />
      </LPBuilderLayout>
    </LPBuilderProvider>
  );
}